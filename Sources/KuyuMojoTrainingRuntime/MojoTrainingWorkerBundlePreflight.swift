...
/// Adapts Mojo runtime-library verification to Kuyu's outer worker-bundle
/// preflight without making the accelerator library the worker entry point.
  ...
  TrainingRunWorkerExecutableBundlePreflighting, Sendable
{
  ...
  private let requirement: MojoAcceleratorRuntimeBundleRequirement
  private let acceleratorPreflight: any MojoAcceleratorRuntimeBundlePreflighting
  ...
  public func validatedAcceleratorRuntime(
    ...
    let bundle = try acceleratorPreflight.validatedRuntimeBundle(
      at: expectedRootURL,
    ...
    let expectedLibraryURL = expectedRootURL.appendingPathComponent(
      bundle.verification.library.relativePath,
      isDirectory: false
    ).standardizedFileURL
    let actualLibraryURL = bundle.libraryURL.standardizedFileURL
    guard actualLibraryURL == expectedLibraryURL else {
      throw
        MojoTrainingWorkerBundlePreflightError
        .acceleratorLibraryURLMismatch(
          expected: expectedLibraryURL,
          actual: actualLibraryURL
        )
    }
    return bundle
  }
